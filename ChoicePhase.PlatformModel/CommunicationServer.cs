...
namespace ChoicePhase.PlatformModel
{
    public class CommunicationServer : ObservableObject
    {
       ...
       public UdpClientManager UdpMonitorServer
       {
        ...
        /// 初始化通讯服务
        /// </summary>
        //public CommunicationServer(byte downAddress,IPAddress ip,int port, Action<Exception> actionException):this(downAddress)
        //{
            
            

        //}
        public void UdpServerInit(IPAddress ip,int port, Action<Exception> actionException)
        {
            _udpServer = new UdpClientManager(ip, port, actionException);
        }
